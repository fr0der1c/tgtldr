...
func (s *Service) maybeSendKeywordAlert(ctx context.Context, chat model.Chat, item model.Message) {
	...
	if !s.canSendKeywordAlert(chat.ID, matched) {
		return
	}
	...
	s.markKeywordAlertSent(chat.ID, matched)
}
...
func (s *Service) canSendKeywordAlert(chatID int64, keyword string) bool {
...
func (s *Service) markKeywordAlertSent(chatID int64, keyword string) {
	key := strconv.FormatInt(chatID, 10) + "|" + strings.ToLower(strings.TrimSpace(keyword))

	s.alertMu.Lock()
	defer s.alertMu.Unlock()
	s.alertLastSent[key] = s.clock.Now()
}
